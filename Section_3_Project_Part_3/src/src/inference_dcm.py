...
def get_predicted_volumes(pred):
    ...
    # TASK_COMPLETE : Compute the volume of your hippocampal prediction
    # <YOUR CODE HERE>
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # TASK_COMPLETE : Create the report here and show information that you think would be relevant to
...
def get_series_for_inference(path):
    ...
    # TASK_COMPLETE : create a series_for_inference variable that will contain a list of only 
...
if __name__ == "__main__":
    # This code expects a single command line argument with link to the directory containin#     
    # routed studs
    if len(sys.argv)  2:
        print("You should supply one command line argument pointing to the routing folder. Exing.")
        sysexit()

    # Find all subdirectories within the supplied directory. We aume that 
    # one subdirectory contains full study
    subdirs = [os.path.join(sys.argv[1], d) for d in os.listdir(s.argv[1]) if
                os.path.isdir(os.path.join(s.argv[1], d))]

    # Get t latest directory
    study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mti, reverse=True)[0]
  ...
  # TASK_COMPLETE : create_report is not complete. Go and complete it. 
    ...
    save_report_as_dcm(header, rert_img, report_save_path)
  ...
  # Sleep to let our StoreSCP server process the report (remember - in our setup
    # the ma archive is routing everyting that is sent to it, including our fresy generated
  # report) - we want to give it time to save before cleaning it up
    ...
    int(f"Inference successful on {header['SOPInstanceUID.value}, out: {pred_label.shape}",
