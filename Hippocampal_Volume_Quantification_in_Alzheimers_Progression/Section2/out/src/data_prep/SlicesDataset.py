...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TASK_COMPLETED: Create two new keys in the "sample" dictionary, named "image" and "seg"
...
#         img = torch.from_numpy(self.data[slc[0]["image"][slc[1]]]).unsqueeze(0).unsqueeze(0).to(device)
#         ## ?? Adding size -1 ??
#         sample["image"] = img[None, :]
#         ## ?? Adding size -1 ??
#         seg = torch.from_numpy(self.data[slc[0]["seg"][slc[1]]]).unsqueeze(0).unsqueeze(0).to(device)
#         sample["seg"] = seg[None, :]
