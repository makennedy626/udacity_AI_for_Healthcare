...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        # Create data loaders
        # TASK_COMPLETED: SlicesDataset class is not complete. Go to the file and complete it. 
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ?? TASK_COMPLETED ??: You have your data in batch variable. Put the slices as 4D Torch Tensors of 
            # shape [BATCH_SIZE, 1, PATCH_SIZE, PATCH_SIZE] into variables data and target. 
            # Feed data to the model and feed target to the loss function
            # 
            data = batch["image"].float()
#             data = data[self.train_loader.batch_size, 1, self.train_loader.patch_size, self.train_loader.patch_size]
            target = batch["seg"]
#             target = target[self.config.batch_size, 1, self.config.patch_size, self.config.patch_size]
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                # TASK_COMPLETED: Write validation code that will compute loss on a validation sample
                # <YOUR CODE HERE>
                ## Might need to implement the reshaping like in the train section
                data = batch["image"].float()
#                 data = data[config.batch_size, 1, config.patch_size, config.patch_size]
                target = batch["seg"]
#                 target = target[config.batch_size, 1, config.patch_size, config.patch_size]
    ...
    def run_test(self):
        ...
        # TASK_COMPLETED: Inference Agent is not complete. Go and finish it. Feel free to test the class
