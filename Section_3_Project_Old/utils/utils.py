...
def med_reshape(image, new_shape):
    ...
    # TASK_COMPLETED: write your original image into the reshaped image
    # <CODE GOES HERE>
    ## From Segmentation Hands-on Solution
    for slc_ix in range(reshaped_image.shape[2]):
        reshaped_image[:,:,slc_ix] = np.argmax(image[:,:,slc_ix])
