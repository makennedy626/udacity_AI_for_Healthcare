...
def cast_df(df, col, d_type=str):
    ...
    df[col] = np.where(pd.isnull(df[col]), df[col], df[col].astype(d_type))
    return df[col]
...
#     df[predictor] = df[predictor].astype(float)
    for c in categorical_col_list:
        df.loc[:, c] = cast_df(df, c, d_type=str)
#         df[c] = cast_df(df, c, d_type=str)
    for numerical_column in numerical_col_list:
        df.loc[:, numerical_column] = impute_df(df, numerical_column, numerical_impute_value)
